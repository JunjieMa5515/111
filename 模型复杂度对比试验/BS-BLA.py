...
def train_model(params):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets in train_loader:
            ...
            optimizer.zero_grad()
            outputs, _ = model(inputs)  # 返回两个值，但这里只使用预测值
        ...
        if val_loss <= target_loss:
            ...
            print(f'Target validation loss reached at epoch {epoch + 1}')
    ...
    for i, t in enumerate(epoch_times):
        print(f'Time for epoch {i + 1}: {t:.2f} seconds')

    return {'loss': best_val_loss, 'status': STATUS_OK}

# 定义超参数空间
space = {
    'hidden_dim': hp.quniform('hidden_dim', 10, 400, 1),
    'learning_rate': hp.loguniform('learning_rate', -5, -1),
    'batch_size': hp.quniform('batch_size', 16, 400, 1)
}
...
def custom_fmin(fn, space, algo, max_evals, trials, early_stop_loss):
    ...
    for i in range(max_evals):
        ...
        print(f"Iteration {i+1} completed in {iteration_time:.2f} seconds with loss: {best_loss:.6f}")
        if best_loss <= early_stop_loss:
            print(f"Early stopping as best loss reached {best_loss:.6f} at iteration {i+1}")
            break
    print(f'Total BO time: {total_iteration_time:.2f} seconds')
    return best

# 调用自定义的fmin函数
early_stop_loss = 0.01  # 设置目标损失值
trials = Trials()
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        val_output, attention_weights = model(valid_X)
        val_loss = criterion(val_output, valid_y).item()
...
model.eval()
with torch.no_grad():
    predictions, attention_weights = model(test_X)  # 返回两个值
