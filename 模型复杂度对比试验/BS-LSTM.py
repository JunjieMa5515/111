...
# 小波降噪函数
def wavelet_denoising(data, wavelet='db4', level=1, threshold_scale=0.52):
...
class BiLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers):
        ...
        self.lstm = nn.LSTM(input_size=input_dim, hidden_size=hidden_dim, num_layers=n_layers, bidirectional=False, batch_first=True)
        self.dropout2 = nn.Dropout(0.2)
        self.fc1 = nn.Linear(hidden_dim  * n_hours, 32)
...
def train_model(params):
    ...
    for epoch in range(num_epochs):
        ...
        model.eval()
        with torch.no_grad():
            val_output = model(valid_X)  # 只接收预测值
        ...
        if epochs_no_improve >= early_stopping_patience:
            ...
            print(f'Total time taken for training until early stopping: {elapsed_time:.2f} seconds')
    ...
    if timer.start_time is not None:
        elapsed_time = timer.stop()
        print(f'Total time taken for training 未触发: {elapsed_time:.2f} seconds')
...
model.eval()
with torch.no_grad():
    predictions = model(test_X)  # 返回两个值
