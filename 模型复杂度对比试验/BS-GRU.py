...
normalized_data = scaler.fit_transform(values123)
...
class GRU(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers):
        ...
        self.gru = nn.GRU(input_size=input_dim, hidden_size=hidden_dim, num_layers=n_layers, bidirectional=True, batch_first=True)
...
def train_model(params):
    ...
    model = GRU(n_features, hidden_dim, 1, n_layers).to(device)  # 将模型移到GPU
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets in train_loader:
            ...
            threshold = 0.8
            lowhold = 0.3
            weights[targets > threshold] = 2  # 对峰值部分的权重设置为2.0
...
model = GRU(n_features, best_hidden_dim, 1, n_layers).to(device)  # 将模型移到GPU
criterion = nn.MSELoss().to(device)
